// Recorded 2026-09-21. Long fields that the app does not read are cut.
enum RDAPFixtures {
    static let dnsBootstrap = """
{
  "description": "RDAP bootstrap file for Domain Name System registrations",
  "publication": "2026-09-16T19:00:03Z",
  "services": [
    [
      [
        "uk"
      ],
      [
        "https://rdap.nominet.uk/uk/"
      ]
    ],
    [
      [
        "com"
      ],
      [
        "https://rdap.verisign.com/com/v1/"
      ]
    ],
    [
      [
        "net"
      ],
      [
        "https://rdap.verisign.com/net/v1/"
      ]
    ]
  ],
  "version": "1.0"
}
"""
    static let ipv4Bootstrap = """
{
  "description": "RDAP bootstrap file for IPv4 address allocations",
  "publication": "2019-06-07T19:00:02Z",
  "services": [
    [
      [
        "41.0.0.0/8",
        "102.0.0.0/8",
        "105.0.0.0/8",
        "154.0.0.0/8",
        "196.0.0.0/8",
        "197.0.0.0/8"
      ],
      [
        "https://rdap.afrinic.net/rdap/",
        "http://rdap.afrinic.net/rdap/"
      ]
    ],
    [
      [
        "1.0.0.0/8",
        "14.0.0.0/8",
        "27.0.0.0/8",
        "36.0.0.0/8",
        "39.0.0.0/8",
        "42.0.0.0/8",
        "43.0.0.0/8",
        "49.0.0.0/8",
        "58.0.0.0/8",
        "59.0.0.0/8",
        "60.0.0.0/8",
        "61.0.0.0/8",
        "101.0.0.0/8",
        "103.0.0.0/8",
        "106.0.0.0/8",
        "110.0.0.0/8",
        "111.0.0.0/8",
        "112.0.0.0/8",
        "113.0.0.0/8",
        "114.0.0.0/8",
        "115.0.0.0/8",
        "116.0.0.0/8",
        "117.0.0.0/8",
        "118.0.0.0/8",
        "119.0.0.0/8",
        "120.0.0.0/8",
        "121.0.0.0/8",
        "122.0.0.0/8",
        "123.0.0.0/8",
        "124.0.0.0/8",
        "125.0.0.0/8",
        "126.0.0.0/8",
        "133.0.0.0/8",
        "150.0.0.0/8",
        "153.0.0.0/8",
        "163.0.0.0/8",
        "171.0.0.0/8",
        "175.0.0.0/8",
        "180.0.0.0/8",
        "182.0.0.0/8",
        "183.0.0.0/8",
        "202.0.0.0/8",
        "203.0.0.0/8",
        "210.0.0.0/8",
        "211.0.0.0/8",
        "218.0.0.0/8",
        "219.0.0.0/8",
        "220.0.0.0/8",
        "221.0.0.0/8",
        "222.0.0.0/8",
        "223.0.0.0/8"
      ],
      [
        "https://rdap.apnic.net/"
      ]
    ],
    [
      [
        "3.0.0.0/8",
        "4.0.0.0/8",
        "6.0.0.0/8",
        "7.0.0.0/8",
        "8.0.0.0/8",
        "9.0.0.0/8",
        "11.0.0.0/8",
        "12.0.0.0/8",
        "13.0.0.0/8",
        "15.0.0.0/8",
        "16.0.0.0/8",
        "17.0.0.0/8",
        "18.0.0.0/8",
        "19.0.0.0/8",
        "20.0.0.0/8",
        "21.0.0.0/8",
        "22.0.0.0/8",
        "23.0.0.0/8",
        "24.0.0.0/8",
        "26.0.0.0/8",
        "28.0.0.0/8",
        "29.0.0.0/8",
        "30.0.0.0/8",
        "32.0.0.0/8",
        "33.0.0.0/8",
        "34.0.0.0/8",
        "35.0.0.0/8",
        "38.0.0.0/8",
        "40.0.0.0/8",
        "44.0.0.0/8",
        "45.0.0.0/8",
        "47.0.0.0/8",
        "48.0.0.0/8",
        "50.0.0.0/8",
        "52.0.0.0/8",
        "54.0.0.0/8",
        "55.0.0.0/8",
        "56.0.0.0/8",
        "63.0.0.0/8",
        "64.0.0.0/8",
        "65.0.0.0/8",
        "66.0.0.0/8",
        "67.0.0.0/8",
        "68.0.0.0/8",
        "69.0.0.0/8",
        "70.0.0.0/8",
        "71.0.0.0/8",
        "72.0.0.0/8",
        "73.0.0.0/8",
        "74.0.0.0/8",
        "75.0.0.0/8",
        "76.0.0.0/8",
        "96.0.0.0/8",
        "97.0.0.0/8",
        "98.0.0.0/8",
        "99.0.0.0/8",
        "100.0.0.0/8",
        "104.0.0.0/8",
        "107.0.0.0/8",
        "108.0.0.0/8",
        "128.0.0.0/8",
        "129.0.0.0/8",
        "130.0.0.0/8",
        "131.0.0.0/8",
        "132.0.0.0/8",
        "134.0.0.0/8",
        "135.0.0.0/8",
        "136.0.0.0/8",
        "137.0.0.0/8",
        "138.0.0.0/8",
        "139.0.0.0/8",
        "140.0.0.0/8",
        "142.0.0.0/8",
        "143.0.0.0/8",
        "144.0.0.0/8",
        "146.0.0.0/8",
        "147.0.0.0/8",
        "148.0.0.0/8",
        "149.0.0.0/8",
        "152.0.0.0/8",
        "155.0.0.0/8",
        "156.0.0.0/8",
        "157.0.0.0/8",
        "158.0.0.0/8",
        "159.0.0.0/8",
        "160.0.0.0/8",
        "161.0.0.0/8",
        "162.0.0.0/8",
        "164.0.0.0/8",
        "165.0.0.0/8",
        "166.0.0.0/8",
        "167.0.0.0/8",
        "168.0.0.0/8",
        "169.0.0.0/8",
        "170.0.0.0/8",
        "172.0.0.0/8",
        "173.0.0.0/8",
        "174.0.0.0/8",
        "184.0.0.0/8",
        "192.0.0.0/8",
        "198.0.0.0/8",
        "199.0.0.0/8",
        "204.0.0.0/8",
        "205.0.0.0/8",
        "206.0.0.0/8",
        "207.0.0.0/8",
        "208.0.0.0/8",
        "209.0.0.0/8",
        "214.0.0.0/8",
        "215.0.0.0/8",
        "216.0.0.0/8"
      ],
      [
        "https://rdap.arin.net/registry/",
        "http://rdap.arin.net/registry/"
      ]
    ],
    [
      [
        "2.0.0.0/8",
        "5.0.0.0/8",
        "25.0.0.0/8",
        "31.0.0.0/8",
        "37.0.0.0/8",
        "46.0.0.0/8",
        "51.0.0.0/8",
        "53.0.0.0/8",
        "57.0.0.0/8",
        "62.0.0.0/8",
        "77.0.0.0/8",
        "78.0.0.0/8",
        "79.0.0.0/8",
        "80.0.0.0/8",
        "81.0.0.0/8",
        "82.0.0.0/8",
        "83.0.0.0/8",
        "84.0.0.0/8",
        "85.0.0.0/8",
        "86.0.0.0/8",
        "87.0.0.0/8",
        "88.0.0.0/8",
        "89.0.0.0/8",
        "90.0.0.0/8",
        "91.0.0.0/8",
        "92.0.0.0/8",
        "93.0.0.0/8",
        "94.0.0.0/8",
        "95.0.0.0/8",
        "109.0.0.0/8",
        "141.0.0.0/8",
        "145.0.0.0/8",
        "151.0.0.0/8",
        "176.0.0.0/8",
        "178.0.0.0/8",
        "185.0.0.0/8",
        "188.0.0.0/8",
        "193.0.0.0/8",
        "194.0.0.0/8",
        "195.0.0.0/8",
        "212.0.0.0/8",
        "213.0.0.0/8",
        "217.0.0.0/8"
      ],
      [
        "https://rdap.db.ripe.net/"
      ]
    ],
    [
      [
        "177.0.0.0/8",
        "179.0.0.0/8",
        "181.0.0.0/8",
        "186.0.0.0/8",
        "187.0.0.0/8",
        "189.0.0.0/8",
        "190.0.0.0/8",
        "191.0.0.0/8",
        "200.0.0.0/8",
        "201.0.0.0/8"
      ],
      [
        "https://rdap.lacnic.net/rdap/"
      ]
    ]
  ],
  "version": "1.0"
}
"""
    static let ipv6Bootstrap = """
{
  "description": "RDAP bootstrap file for IPv6 address allocations",
  "publication": "2024-11-01T22:00:01Z",
  "services": [
    [
      [
        "2001:4200::/23",
        "2c00::/12"
      ],
      [
        "https://rdap.afrinic.net/rdap/",
        "http://rdap.afrinic.net/rdap/"
      ]
    ],
    [
      [
        "2001:200::/23",
        "2001:4400::/23",
        "2001:8000::/19",
        "2001:a000::/20",
        "2001:b000::/20",
        "2001:c00::/23",
        "2001:e00::/23",
        "2400::/12",
        "2410::/12"
      ],
      [
        "https://rdap.apnic.net/"
      ]
    ],
    [
      [
        "2001:1800::/23",
        "2001:400::/23",
        "2001:4800::/23",
        "2600::/12",
        "2610::/23",
        "2620::/23",
        "2630::/12"
      ],
      [
        "https://rdap.arin.net/registry/",
        "http://rdap.arin.net/registry/"
      ]
    ],
    [
      [
        "2001:1400::/22",
        "2001:1a00::/23",
        "2001:1c00::/22",
        "2001:2000::/19",
        "2001:4000::/23",
        "2001:4600::/23",
        "2001:4a00::/23",
        "2001:4c00::/23",
        "2001:5000::/20",
        "2001:600::/23",
        "2001:800::/22",
        "2003::/18",
        "2a00::/12",
        "2a10::/12"
      ],
      [
        "https://rdap.db.ripe.net/"
      ]
    ],
    [
      [
        "2001:1200::/23",
        "2800::/12"
      ],
      [
        "https://rdap.lacnic.net/rdap/"
      ]
    ]
  ],
  "version": "1.0"
}
"""
    static let asnBootstrap = """
{
  "description": "RDAP bootstrap file for Autonomous System Number allocations",
  "publication": "2026-06-01T20:00:01Z",
  "services": [
    [
      [
        "36864-37887",
        "327680-328703",
        "328704-329727",
        "329728-330751"
      ],
      [
        "https://rdap.afrinic.net/rdap/",
        "http://rdap.afrinic.net/rdap/"
      ]
    ],
    [
      [
        "4608-4865",
        "7467-7722",
        "9216-10239",
        "17408-18431",
        "23552-24575",
        "37888-38911",
        "45056-46079",
        "55296-56319",
        "58368-59391",
        "63488-63999",
        "64000-64098",
        "64297-64395",
        "131072-132095",
        "132096-133119",
        "133120-133631",
        "133632-134556",
        "134557-135580",
        "135581-136505",
        "136506-137529",
        "137530-138553",
        "138554-139577",
        "139578-140601",
        "140602-141625",
        "141626-142649",
        "142650-143673",
        "143674-144697",
        "144698-145721",
        "145722-146745",
        "146746-147769",
        "147770-148793",
        "148794-149817",
        "149818-150841",
        "150842-151865",
        "151866-152889",
        "152890-153913",
        "153914-154937",
        "154938-155961"
      ],
      [
        "https://rdap.apnic.net/"
      ]
    ],
    [
      [
        "1-1876",
        "1902-2042",
        "2044-2046",
        "2048-2106",
        "2137-2584",
        "2615-2772",
        "2823-2829",
        "2880-3153",
        "3354-4607",
        "4866-5376",
        "5632-6655",
        "6912-7466",
        "7723-8191",
        "10240-12287",
        "13312-15359",
        "16384-17407",
        "18432-20479",
        "21504-23455",
        "23457-23551",
        "25600-26623",
        "26624-27647",
        "29696-30719",
        "31744-32767",
        "32768-33791",
        "35840-36863",
        "39936-40959",
        "46080-47103",
        "53248-54271",
        "54272-55295",
        "62464-63487",
        "64198-64296",
        "393216-394239",
        "394240-395164",
        "395165-396188",
        "396189-397212",
        "397213-398236",
        "398237-399260",
        "399261-400284",
        "400285-401308",
        "401309-402332",
        "402333-403356",
        "403357-404380"
      ],
      [
        "https://rdap.arin.net/registry/",
        "http://rdap.arin.net/registry/"
      ]
    ],
    [
      [
        "1877-1901",
        "2043",
        "2047",
        "2107-2136",
        "2585-2614",
        "2773-2822",
        "2830-2879",
        "3154-3353",
        "5377-5631",
        "6656-6911",
        "8192-9215",
        "12288-13311",
        "15360-16383",
        "20480-21503",
        "24576-25599",
        "28672-29695",
        "30720-31743",
        "33792-34815",
        "34816-35839",
        "38912-39935",
        "40960-41983",
        "41984-43007",
        "43008-44031",
        "44032-45055",
        "47104-48127",
        "48128-49151",
        "49152-50175",
        "50176-51199",
        "51200-52223",
        "56320-57343",
        "57344-58367",
        "59392-60415",
        "60416-61439",
        "61952-62463",
        "64396-64495",
        "196608-197631",
        "197632-198655",
        "198656-199679",
        "199680-200191",
        "200192-201215",
        "201216-202239",
        "202240-203263",
        "203264-204287",
        "204288-205211",
        "205212-206235",
        "206236-207259",
        "207260-208283",
        "208284-209307",
        "209308-210331",
        "210332-211355",
        "211356-212379",
        "212380-213403",
        "213404-214427",
        "214428-215451",
        "215452-216475",
        "216476-217499",
        "217500-218523",
        "218524-219547"
      ],
      [
        "https://rdap.db.ripe.net/"
      ]
    ],
    [
      [
        "27648-28671",
        "52224-53247",
        "61440-61951",
        "64099-64197",
        "262144-263167",
        "263168-263679",
        "263680-264604",
        "264605-265628",
        "265629-266652",
        "266653-267676",
        "267677-268700",
        "268701-269724",
        "269725-270748",
        "270749-271772",
        "271773-272796",
        "272797-273820",
        "273821-274844",
        "274845-275868"
      ],
      [
        "https://rdap.lacnic.net/rdap/"
      ]
    ]
  ],
  "version": "1.0"
}
"""
    static let domain = """
{
  "objectClassName": "domain",
  "handle": "2336799_DOMAIN_COM-VRSN",
  "ldhName": "EXAMPLE.COM",
  "status": [
    "client delete prohibited",
    "client transfer prohibited",
    "client update prohibited"
  ],
  "entities": [
    {
      "objectClassName": "entity",
      "handle": "376",
      "roles": [
        "registrar"
      ],
      "vcardArray": [
        "vcard",
        [
          [
            "version",
            {},
            "text",
            "4.0"
          ],
          [
            "fn",
            {},
            "text",
            "RESERVED-Internet Assigned Numbers Authority"
          ]
        ]
      ],
      "entities": [
        {
          "objectClassName": "entity",
          "roles": [
            "abuse"
          ],
          "vcardArray": [
            "vcard",
            [
              [
                "version",
                {},
                "text",
                "4.0"
              ],
              [
                "fn",
                {},
                "text",
                ""
              ],
              [
                "tel",
                {
                  "type": "voice"
                },
                "uri",
                ""
              ],
              [
                "email",
                {},
                "text",
                ""
              ]
            ]
          ]
        }
      ]
    }
  ],
  "events": [
    {
      "eventAction": "registration",
      "eventDate": "1995-08-14T04:00:00Z"
    },
    {
      "eventAction": "expiration",
      "eventDate": "2027-08-13T04:00:00Z"
    },
    {
      "eventAction": "last changed",
      "eventDate": "2026-08-14T08:01:43Z"
    },
    {
      "eventAction": "last update of RDAP database",
      "eventDate": "2026-09-21T10:16:34Z"
    }
  ],
  "nameservers": [
    {
      "objectClassName": "nameserver",
      "ldhName": "ELLIOTT.NS.CLOUDFLARE.COM"
    },
    {
      "objectClassName": "nameserver",
      "ldhName": "HERA.NS.CLOUDFLARE.COM"
    }
  ],
  "rdapConformance": [
    "rdap_level_0",
    "icann_rdap_technical_implementation_guide_1",
    "icann_rdap_response_profile_1"
  ]
}
"""
    static let ip = """
{
  "handle": "193.0.0.0 - 193.0.7.255",
  "startAddress": "193.0.0.0",
  "endAddress": "193.0.7.255",
  "ipVersion": "v4",
  "name": "RIPE-NCC",
  "type": "ASSIGNED PA",
  "country": "NL",
  "parentHandle": "193.0.0.0 - 193.0.23.255",
  "cidr0_cidrs": [
    {
      "v4prefix": "193.0.0.0",
      "length": 21
    }
  ],
  "status": [
    "active"
  ],
  "events": [
    {
      "eventAction": "registration",
      "eventDate": "2003-03-17T12:15:57Z"
    },
    {
      "eventAction": "last changed",
      "eventDate": "2026-03-19T09:08:35Z"
    }
  ],
  "rdapConformance": [
    "geofeed1",
    "rirSearch1",
    "ips",
    "cidr0",
    "rdap_level_0",
    "nro_rdap_profile_0",
    "redacted"
  ],
  "port43": "whois.ripe.net",
  "objectClassName": "ip network"
}
"""
    static let asn = """
{
  "handle": "AS3333",
  "startAutnum": 3333,
  "endAutnum": 3333,
  "name": "RIPE-NCC-AS",
  "status": [
    "active"
  ],
  "events": [
    {
      "eventAction": "registration",
      "eventDate": "2002-08-13T12:58:13Z"
    },
    {
      "eventAction": "last changed",
      "eventDate": "2026-03-19T09:33:32Z"
    }
  ],
  "rdapConformance": [
    "nro_rdap_profile_asn_flat_0",
    "rirSearch1",
    "autnums",
    "cidr0",
    "rdap_level_0",
    "nro_rdap_profile_0",
    "redacted"
  ],
  "port43": "whois.ripe.net",
  "objectClassName": "autnum"
}
"""
}

enum RIPEstatFixtures {
    static let networkInfo = """
{
  "data_call_name": "network-info",
  "data_call_status": "supported",
  "cached": false,
  "status": "ok",
  "status_code": 200,
  "data": {
    "asns": [
      "13335"
    ],
    "prefix": "1.1.1.0/24"
  }
}
"""
    static let asOverview = """
{
  "data_call_name": "as-overview",
  "data_call_status": "supported - based on 2.1",
  "cached": false,
  "status": "ok",
  "status_code": 200,
  "data": {
    "type": "as",
    "resource": "13335",
    "block": {
      "resource": "13312-15359",
      "desc": "Assigned by ARIN",
      "name": "IANA 16-bit Autonomous System (AS) Numbers Registry"
    },
    "holder": "CLOUDFLARENET - Cloudflare, Inc.",
    "announced": true,
    "query_starttime": "2026-09-21T00:00:00",
    "query_endtime": "2026-09-21T00:00:00"
  }
}
"""
    static let networkInfoPrivate = """
{
  "data_call_name": "network-info",
  "data_call_status": "supported",
  "cached": false,
  "status": "ok",
  "status_code": 200,
  "data": {
    "asns": [],
    "prefix": ""
  }
}
"""
    static let networkInfoIPv6 = """
{
  "data_call_name": "network-info",
  "data_call_status": "supported",
  "cached": false,
  "status": "ok",
  "status_code": 200,
  "data": {
    "asns": [
      "13335"
    ],
    "prefix": "2606:4700:4700::/48"
  }
}
"""
    static let asOverviewUnknown = """
{
  "data_call_name": "as-overview",
  "data_call_status": "supported - based on 2.1",
  "cached": false,
  "status": "ok",
  "status_code": 200,
  "data": {
    "type": "as",
    "resource": "64500",
    "block": {
      "resource": "64496-64511",
      "desc": "Reserved for use in documentation and sample code (Reference: RFC5398)",
      "name": "IANA 16-bit Autonomous System (AS) Numbers Registry"
    },
    "holder": "",
    "announced": false,
    "query_starttime": "2026-09-21T00:00:00",
    "query_endtime": "2026-09-21T00:00:00"
  }
}
"""
}
