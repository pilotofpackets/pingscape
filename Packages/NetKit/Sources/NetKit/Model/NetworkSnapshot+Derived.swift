...
extension NetworkSnapshot {
    ...
    public var primaryInterface: NetworkInterface? {
        for kind in Self.primaryOrder {
            if let match = interfaces.first(where: { $0.kind == kind && $0.isUp && $0.hasUsableAddress }) {
                return match
            }
        }
    ...
    public func interface(of kind: InterfaceKind) -> NetworkInterface? {
        interfaces.first { $0.kind == kind && $0.hasUsableAddress }
    }

    /// Whether an interface is a VPN tunnel the user (or an app) set up.
    ///
    /// `utun` and `ppp` with a usable address always count. iOS creates a few
    /// system `utun` interfaces, but those carry only link-local addresses.
    /// `ipsec` counts only if the system lists it as a network service.
    ...
    private func localGateway(ipv6: Bool) -> String? {
        guard !defaultRoutes.isEmpty, let primary = primaryInterface else { return nil }
    ...
    /// The network of the primary interface, used for the LAN sweep.
    public var lanRange: IPv4Range? {
        guard let address = primaryIPv4 else { return nil }
